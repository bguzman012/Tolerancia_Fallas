...
@Component({
  selector: 'app-subproceso',
  templateUrl: './subproceso.component.html',
  styleUrls: ['./subproceso.component.scss']
})
export class SubprocesoComponent implements OnInit {
  ...
  getData(page, limit, search) {
    ...
    this.riesgosServices.getSubProcesos(page == null ? 0 : page, limit == null ? 10 : limit, search == null ? '' : search).then(data => {
  ...
  openDialog(data) {
    ...
    const modalRef = this.modalService.open(SubprocesoDialogComponent, { ariaLabelledBy: 'Ingreso de Cliente', size: 'xl' });
  ...
  deleteItem(item) {
    ...
    Swal.fire({
      title: 'Eliminar',
      text: "¿Está seguro de eliminar el subproceso?",
      icon: 'warning',
      ...
      /** */
      this.riesgosServices.deleteSubProceso(item.id_subproceso).subscribe(resp => {
